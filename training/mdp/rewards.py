...
def _T(x):
    """09-08 新机合并：IsaacLab 6.x 的 asset.data.* 是 ProxyArray（warp 背书），送进 TorchScript 的四元数函数前转成 torch 张量。"""
...
def track_lin_vel_xy_yaw_frame_exp(
    ...
    vel_yaw = quat_apply_inverse(yaw_quat(_T(asset.data.root_quat_w)), asset.data.root_lin_vel_w[:, :3])
    lin_vel_error = torch.sum(
...
def joint_pos_penalty_except_turn_side_cmd(
    ...
    if sensor_cfg is not None:
        ...
        if phase_exempt:
            # C18：豁免门从"地形高"换成"正在翻越"。33 cm 台面本身算高地形，
            # 原门让机器人在台面上行驶的全程都只挨 1/10 的姿态罚（见 climb_exempt_mask 的实测）。
...
def feet_distance_y_exp(
    ...
    for i in range(n_feet):
        footsteps_in_body_frame[:, i, :] = math_utils.quat_apply(
            math_utils.quat_conjugate(_T(asset.data.root_link_quat_w)), cur_footsteps_translated[:, i, :]
        )
...
def feet_height_body(
    ...
    for i in range(len(asset_cfg.body_ids)):
        footpos_in_body_frame[:, i, :] = math_utils.quat_apply_inverse(
            _T(asset.data.root_quat_w), cur_footpos_translated[:, i, :]
        )
        footvel_in_body_frame[:, i, :] = math_utils.quat_apply_inverse(
            _T(asset.data.root_quat_w), cur_footvel_translated[:, i, :]
        )
...
def phase_foot_trajectory_exp(
    ...
    if (not hasattr(env, "phase_foot_ref_body")) or (env.phase_foot_ref_body.shape[1] != num_feet):
        ...
        for i in range(num_feet):
            foot_pos_b[:, i, :] = math_utils.quat_apply_inverse(_T(asset.data.root_quat_w), rel_foot_pos_w[:, i, :])
    ...
    for i in range(num_feet):
        foot_pos_b[:, i, :] = math_utils.quat_apply_inverse(_T(asset.data.root_quat_w), rel_foot_pos_w[:, i, :])
        foot_vel_b[:, i, :] = math_utils.quat_apply_inverse(_T(asset.data.root_quat_w), rel_foot_vel_w[:, i, :])
