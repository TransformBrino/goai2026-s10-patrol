...
def randomize_rigid_body_inertia(
    ...
    # 09-08 新机合并：IsaacLab 6.x 的 physx_view 返回只读 warp 数组，改从 asset.data 取 torch 张量（参照 isaaclab.envs.mdp.events 6.1）
    inertias = asset.data.body_inertia.torch.clone()

    # apply randomization on default values
    inertias[env_ids[:, None], body_ids, :] = asset.data.default_inertia.torch[env_ids[:, None], body_ids, :].clone()
    ...
    # set the inertia tensors into the physics simulation (6.x: 资产级 setter，只传 (len(env_ids), len(body_ids), 9) 的局部张量)
    dev = inertias.device
    asset.set_inertias_index(inertias=inertias[env_ids[:, None], body_ids], body_ids=body_ids.to(dev), env_ids=env_ids.to(dev))
