...
private:
    ...
    double time_stamp_record_, run_time_;   // 0911g：同上，绝对时间戳要用 double
    ...
    float GetHipYPosByHeight(float h){
        ...
        return LimitNumber(theta, cp_ptr_->fl_joint_lower_(1), cp_ptr_->fl_joint_upper_(1));   // 0911g：LimitNumber 是传值返回，原来丢了返回值、限位没生效
    }
    ...
    float GetKneePosByHeight(float h){
        ...
        return LimitNumber(theta, cp_ptr_->fl_joint_lower_(2), cp_ptr_->fl_joint_upper_(2));   // 0911g：同上
    }
    ...
    virtual StateName GetNextStateName() {
        ...
        if(run_time_ - time_stamp_record_ <= 2.*liedown_duration_){
            ...
            if(uc_ptr_->GetUserCommand()->target_mode == uint8_t(RobotMotionState::StandingUp)){
                if(!imu_gate::Ok(&*ri_ptr_, "起立", imu_gate_print_)) return StateName::kLieDown;   // 0911d
