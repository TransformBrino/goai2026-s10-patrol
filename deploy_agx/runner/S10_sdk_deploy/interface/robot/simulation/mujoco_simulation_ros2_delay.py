...
# 关节指令执行延迟（仿真专用，默认 0 = 与原版逐位相同）。真机控制回路有几十 ms 延迟，训练里随机 0~3 个策略步（0~60 ms）。
CMD_DELAY_S = float(os.environ.get("S10_CMD_DELAY_MS", "0") or 0) / 1000.0
...
class MuJoCoSimulationNode(Node):
    def __init__(self,
        ...
        if CMD_DELAY_S > 0.0:
            print(f"[延迟] 关节指令执行延迟 {CMD_DELAY_S*1000:.0f} ms（S10_CMD_DELAY_MS）", flush=True)
    ...
    def _cmd_callback(self, msg: JointsDataCmd):
        ...
        for i in range(self.dof_num):
            ...
            tau[i] = joint_cmd.torque  # tau_ff no processing

        # Convert: raw = published * dir + offset_rad
        pos = pub_pos * JOINT_DIR + POS_OFFSET_RAD
        vel = pub_vel * JOINT_DIR
        if CMD_DELAY_S > 0.0:
            # 按到达时的仿真时刻入队，满 CMD_DELAY_S 才生效（主循环出队）
            self._cmd_q.append((getattr(self, "timestamp", 0.0), kp, kd, pos, vel, tau))
            return
        self._set_cmd(kp, kd, pos, vel, tau)

    def _set_cmd(self, kp, kd, pos, vel, tau):
        self.kp_cmd.flat = kp
        self.kd_cmd.flat = kd
        self.pos_cmd.flat = pos
        self.vel_cmd.flat = vel
        self.tau_ff.flat = tau
    ...
    def start(self):
        ...
        while rclpy.ok():
            ...
            step += 1
            # 控制律
            if CMD_DELAY_S > 0.0 and self._cmd_q:
                _now = getattr(self, "timestamp", 0.0)
                _latest = None
                while self._cmd_q and self._cmd_q[0][0] + CMD_DELAY_S <= _now + 1e-9:
                    _latest = self._cmd_q.popleft()
                if _latest is not None:
                    self._set_cmd(*_latest[1:])
