...
- 定位：FAST-LIO2 ROS 2 分支（Ericsii fork，上游 https://github.com/hku-mars/FAST_LIO.git，本地源码 commit a4743b095409；AGX 部署版本快照见 perception/FAST_LIO_src，含本队二次开发 diff：Airy 双雷达合并云预处理、去 Python 依赖），配置 `deploy_agx/lio/config/s10_airy.yaml`（lio_up.sh 默认），备选 s10_airy_light / s10_airy_diag；IMU 转发 `deploy_agx/lio/s10_imu_relay.py`；kiss-icp 1ffa7d7 已停用
